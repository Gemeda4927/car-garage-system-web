...
import { CreateBookingRequest, CheckAvailabilityRequest, TimeSlot, VehicleInfo } from "@/lib/types/booking.types";
import { PopulatedGarage, GarageService, BusinessHours } from "@/lib/types/garage.types";
...
interface GarageMapProps {
  ...
  onGarageSelect?: (garage: PopulatedGarage) => void;
}

const GarageMap = ({ garages, userLocation, onGarageSelect }: GarageMapProps) => {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<L.Map | null>(null);
  const markers = useRef<L.Marker[]>([]);
  const [mapError, setMapError] = useState<string | null>(null);
  const [mapLoading, setMapLoading] = useState(true);
  const [validGarages, setValidGarages] = useState<PopulatedGarage[]>([]);

  // Filter garages with valid coordinates
  useEffect(() => {
    const valid = garages.filter(garage => 
      garage.coordinates?.coordinates && 
      Array.isArray(garage.coordinates.coordinates) && 
      garage.coordinates.coordinates.length === 2 &&
      !isNaN(garage.coordinates.coordinates[0]) && 
      !isNaN(garage.coordinates.coordinates[1])
    );
    setValidGarages(valid);
    
    if (valid.length === 0 && garages.length > 0) {
      setMapError("No garages have valid location coordinates");
    }
  }, [garages]);
  ...
  useEffect(() => {
    ...
    // Don't initialize map if no user location
    if (!userLocation) {
      setMapLoading(false);
      return;
    }
    ...
    try {
      ...
      // Add OpenStreetMap tiles (completely free, no API key required)
      ...
      L.marker([userLocation.lat, userLocation.lng], { icon: userIcon })
        ...
        .bindPopup(`
          <div class="p-3">
            ...
            <p class="text-xs text-gray-400 mt-1">Lat: ${userLocation.lat.toFixed(4)}, Lng: ${userLocation.lng.toFixed(4)}</p>
          </div>
        `);

      // Add garage markers only if they have valid coordinates
      const bounds = L.latLngBounds([[userLocation.lat, userLocation.lng]]);
      let hasValidGarages = false;

      validGarages.forEach((garage) => {
        if (garage.coordinates?.coordinates) {
          const [lng, lat] = garage.coordinates.coordinates;
          
          // Validate coordinates
          if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
            console.warn(`Invalid coordinates for garage ${garage.name}:`, { lat, lng });
            return;
          }
            ...
            .bindPopup(`
              <div class="p-3 min-w-[220px]">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <span class="text-xs font-medium">${garage.stats.averageRating.toFixed(1)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-2 text-xs text-gray-500 mb-3">
                  ...
                  <span>${garage.contactInfo.phone}</span>
                </div>
                <button 
          ...
          if (onGarageSelect) {
            marker.on('click', () => onGarageSelect(garage));
          }
      ...
      if (hasValidGarages) {
        ...
        // If no valid garages, just center on user location
  ...
  }, [validGarages, userLocation, onGarageSelect]);
...
const ServiceDetailsModal = ({ service, garage, onClose, onBookNow }: ServiceDetailsModalProps) => {
  ...
  const getServiceFeatures = (category: string) => {
    const features: Record<string, Array<{ icon: any; text: string; description: string }>> = {
      "Oil Change": [
        { icon: Droplets, text: "Synthetic Oil", description: "High-quality synthetic oil for better engine performance" },
        { icon: Filter as any, text: "Oil Filter", description: "Premium oil filter replacement included" },
      ...
      "default": [
        { icon: Shield, text: "Professional Service", description: "Certified technicians with years of experience" },
              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{garage.stats.averageRating.toFixed(1)}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <MapPin className="w-4 h-4" />
                    <span>{garage.address.city}</span>
                  </div>
                </div>
              </div>
              <div className="text-right">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-18rem)] scrollbar-thin scrollbar-thumb-gray-300">
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-3 text-center transform hover:scale-105 transition-transform">
              ...
              <p className="text-sm font-bold text-purple-700">{garage.stats.averageRating.toFixed(1)}</p>
            </div>
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-3 text-center transform hover:scale-105 transition-transform">
              ...
              <p className="text-sm font-bold text-amber-700">{garage.stats.totalBookings}</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-4 mb-6 border border-gray-200">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-2 hover:bg-white rounded-lg transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {garage.address.street}, {garage.address.city}, {garage.address.state} {garage.address.zipCode}
                  </p>
                  {garage.coordinates?.coordinates && (
                    <p className="text-xs text-gray-400 mt-1">
                      Lat: {garage.coordinates.coordinates[1].toFixed(4)}, Lng: {garage.coordinates.coordinates[0].toFixed(4)}
                    </p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3 p-2 hover:bg-white rounded-lg transition-colors">
                ...
                <a href={`tel:${garage.contactInfo.phone}`} className="text-sm text-gray-600 hover:text-blue-600 transition-colors">
              ...
              <div className="flex items-center gap-3 p-2 hover:bg-white rounded-lg transition-colors">
                ...
                <a href={`mailto:${garage.contactInfo.email}`} className="text-sm text-gray-600 hover:text-blue-600 transition-colors">
...
const GarageDetailsModal = ({ garage, services, onClose, onServiceSelect, onBookService, loading }: GarageDetailsModalProps) => {
  ...
  const categories = useMemo(() => 
    Array.from(new Set(services.map(s => s.category))), [services]
  );
  ...
  const reviews = [
    { id: 1, user: "John D.", avatar: "JD", rating: 5, comment: "Excellent service! Very professional team. They fixed my car quickly and at a fair price. Will definitely come back.", date: "2 days ago", helpful: 12 },
    { id: 2, user: "Sarah M.", avatar: "SM", rating: 4, comment: "Good work, fair prices. The staff was friendly and explained everything clearly. Only minor delay in service.", date: "1 week ago", helpful: 8 },
    { id: 3, user: "Mike R.", avatar: "MR", rating: 5, comment: "Fast and reliable. Highly recommended! They went above and beyond to help me with my emergency repair.", date: "2 weeks ago", helpful: 15 },
    { id: 4, user: "Emily W.", avatar: "EW", rating: 5, comment: "Best garage in town! Transparent pricing and excellent customer service.", date: "3 weeks ago", helpful: 7 },
    { id: 5, user: "David L.", avatar: "DL", rating: 4, comment: "Very satisfied with the work. Would recommend to friends and family.", date: "1 month ago", helpful: 5 },
  ];
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl animate-scale-in">
        ...
        <div className="relative h-56">
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            <span className={`px-4 py-2 rounded-full text-sm font-medium backdrop-blur flex items-center gap-2 ${
              openStatus 
          ...
          <div className="absolute bottom-4 left-6 text-white">
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-1">
                <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                {garage.stats.averageRating.toFixed(1)} ({garage.stats.totalReviews} reviews)
              </div>
              <div className="flex items-center gap-1">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-20rem)] scrollbar-thin scrollbar-thumb-gray-300">
                ...
                <div className="space-y-3">
                  ...
                  {garage.contactInfo.website && (
                    <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                      ...
                      <a href={garage.contactInfo.website} target="_blank" rel="noopener noreferrer" 
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-600">
                        ...
                        {garage.address.city}, {garage.address.state} {garage.address.zipCode}<br />
                        {garage.address.country}
                      </p>
                      {garage.coordinates?.coordinates && (
                        <div className="mt-2 text-xs text-gray-400 bg-gray-50 p-2 rounded">
                          <p>Latitude: {garage.coordinates.coordinates[1].toFixed(6)}</p>
                          <p>Longitude: {garage.coordinates.coordinates[0].toFixed(6)}</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-r from-yellow-50 via-orange-50 to-red-50 rounded-xl p-6">
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <div className="text-5xl font-bold text-gray-900">{garage.stats.averageRating.toFixed(1)}</div>
                    <div className="flex items-center gap-1 mt-2">
                      {[1,2,3,4,5].map((star) => (
                        <Star key={star} className={`w-5 h-5 ${
                          star <= Math.round(garage.stats.averageRating)
                            ? 'fill-yellow-400 text-yellow-400'
                            : 'text-gray-300'
                        }`} />
                      ))}
                    </div>
                    <p className="text-sm text-gray-500 mt-2">{garage.stats.totalReviews} total reviews</p>
                  </div>
              ...
              <div className="space-y-3">
                {displayedReviews.map((review) => (
                  <div key={review.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-lg transition-shadow bg-white">
                    <div className="flex justify-between items-start mb-3">
          ...
          {activeTab === 'map' && (
            <div className="h-[400px] rounded-xl overflow-hidden">
              <GarageMap 
                garages={[garage]} 
                userLocation={garage.coordinates?.coordinates ? {
...
const GarageCard = ({ garage, onClick }: GarageCardProps) => {
  ...
  const gradientIndex = garage._id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % gradients.length;
  const gradient = gradients[gradientIndex];

  const garageImage = !imageError && garage.images && garage.images.length > 0 
    ? garage.images[0] 
    : null;
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-10 flex gap-2">
          <span className={`px-3 py-1.5 rounded-full text-xs font-medium backdrop-blur flex items-center gap-1 ${
            openStatus 
              ? 'bg-green-500/90 text-white' 
              : 'bg-gray-500/90 text-white'
          }`}>
            <span className={`w-1.5 h-1.5 rounded-full ${openStatus ? 'bg-white animate-pulse' : 'bg-gray-300'}`}></span>
        ...
        <div className="flex items-center gap-2 mt-1 relative z-10">
          <div className="flex items-center bg-black/20 backdrop-blur px-2 py-1 rounded-full">
            ...
            <span className="text-white text-xs font-medium ml-1">{garage.stats.averageRating.toFixed(1)}</span>
          </div>
          <span className="text-white/70 text-xs bg-black/20 backdrop-blur px-2 py-1 rounded-full">
            {garage.stats.totalReviews} reviews
          </span>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">{garage.contactInfo.phone}</span>
          </div>
          <button className={`text-sm font-medium text-blue-600 flex items-center gap-1 transition-all ${
        ...
        {/* Location coordinates badge (for debugging) */}
        {garage.coordinates?.coordinates && (
...
const BookingModal = ({ service, garage, onClose, onConfirm, onCheckAvailability, loading, error }: BookingModalProps) => {
...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "nearby" | "map">("all");
  ...
  // Fetch nearby garages only when explicitly requested
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-10 animate-slide-up">
            ...
            <p className="text-gray-500 text-lg">
              Browse through <span className="font-semibold text-blue-600">{garages.length}+</span> trusted garages and book services instantly
            </p>
          </div>
          ...
          {selectedGarage && !showGarageModal && (
            <button
